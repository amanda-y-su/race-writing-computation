...
# create a table using the alignments data
...
```{r model-tables2}
fit2 <- stan_glmer(social ~ gender + race + bible + race:bible + (1|title), data=model_data, family=binomial, refresh = 0)

table_2 <- tidy(fit2) %>%
  filter(term %in% c("(Intercept)", "gender", "race", "bible", "race:bible")) %>%s
  mutate_if(is.numeric, funs(round(., 3))) 
